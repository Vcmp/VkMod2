...
struct SwapChain
{
    ...
    VkImage image[Frames];
    VkImageView imageViews[Frames];
    VkRenderPass renderpass;
    VkSurfaceFormatKHR         swapChainImageFormat;
    VkPresentModeKHR presentMode;
    VkExtent2D swapChainExtent{width, height};
    VkFramebuffer frameBuffer[Frames];
    SwapChain(/* VkPhysicalDevice physdevice, VkSurfaceKHR surface */)
    {
        setupImageFormats();
        createSwapChain();
        createImageViews();
...
void SwapChain::createSwapChain()
  ...
  void SwapChain::createFramebuffers()
  {
    ...
    for ( size_t i = 0; i < Frames; i++ )
    {
      ...
      Vks::doPointerAlloc<VkFramebuffer, PFN_vkCreateFramebuffer>(VKI.device, &framebufferCreateInfo, &frameBuffer[i],vkCreateFramebuffer);
    }
